...
jQuery.extend(true,_R, {	
	...
	checkForParallax : function(container,opt) {		
		...
		if ((_.type=="3D" || _.type=="3d") && opt.c.find('.tp-assets-layers').length>0) {
			punchgs.TweenLite.set(opt.c.find('.tp-assets-layers'),{top:0, left:0,width:"100%",height:"100%"});
			setDDDInContainer(opt.c.find('.tp-assets-layers'));
		}
		...
		if (_.type=="mouse" || _.type=="scroll+mouse" || _.type=="mouse+scroll" || _.type=="3D" || _.type=="3d") {
			...
			container.on('mousemove.hoverdir, mouseleave.hoverdir, trigger3dpath',function(event,data) {				
				...
				if (_.type=="3D" || _.type=="3d") {
					var sctor = '.tp-revslider-slidesli .dddwrapper, .dddwrappershadow, .tp-revslider-slidesli .dddwrapper-layer, .tp-assets-layers .dddwrapper-layer';
					if (opt.sliderType==="carousel") sctor = ".tp-revslider-slidesli .dddwrapper, .tp-revslider-slidesli .dddwrapper-layer, .tp-assets-layers .dddwrapper-layer";
			...
			if (_ISM)
				window.ondeviceorientation = function(event) {
					...
					container.find('.tp-assets-layers .tp-parallax-container').each(function(){
						pcnts.push(jQuery(this));
					});
